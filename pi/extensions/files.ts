...
const openExternalDiff = (
  ...
  try {
    ...
    const result = spawnSync("nvim", ["-d", leftPath, rightPath], {
      stdio: "inherit",
    });
    ...
    return {
      ok: false,
      error: `nvim exited with status ${result.status ?? "unknown"}`,
    };
  } finally {
    tui.start();
    tui.requestRender(true);
...
const openDiff = async (
  ...
  const diffResult = await ctx.ui.custom<{ ok: boolean; error?: string }>(
    (tui, theme, _kb, done) => {
      const status = new Text(theme.fg("dim", "Opening nvim diff..."));

      queueMicrotask(() => {
        const result = openExternalDiff(tui, tmpFile, workingPath);
        done(result);
      });
